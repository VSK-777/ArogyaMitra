...
export default function PastConsultations() {
    ...
    if (loading) return <div className="flex justify-center p-12"><Loader2 className="w-8 h-8 animate-spin text-blue-700" /></div>;
        ...
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            {consultations.length === 0 ? (
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        {consultations.map((q, idx) => (
                            <div key={idx} 
                                ...
                                className={`p-4 rounded-md border cursor-pointer transition-colors ${selectedAptId === q.appointment?.appointmentId ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                            >
                    ...
                    <div className="bg-white p-6 rounded-md border border-slate-200 shadow-sm h-fit">
                        {selectedAptId ? (
                            <div>
                                <h3 className="font-bold text-lg text-slate-900 mb-4 flex items-center gap-2 border-b pb-2">
                                    <FileText className="h-5 w-5 text-blue-700" /> Medical Records
                                </h3>
